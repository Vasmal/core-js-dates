...
function getTime(date) {
  function twoDigits(num) {
    ...
    return strDigit.length < 2 ? `0${strDigit}` : strDigit;
  }
  const dateObj = date;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (endObj - startObj) / (1000 * 60 * 60 * 24) + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  let targetDate = new Date(newDate.getFullYear(), newDate.getMonth(), 13);
...
function getQuarter(date) {
  ...
  const quarter = (endYear - startYear) / 4;
  let accumulator = quarter;
  let quarterOfyear = 1;

  while (date - startYear > accumulator) {
    accumulator += quarter;
    quarterOfyear += 1;
  }
